					...
					if (frag in target) {
						if (isObject(target[frag])) {
							...
							console.warn(
								`Config key '${
									configKey.key
								}' for experiment '${experimentName}' is overriding non-object value at '${frags
									.slice(0, index + 1)
									.join(".")}' with an object.`
							);
			...
			if (key in target && `_${key}_setter` in Object.getOwnPropertyDescriptor(target, key)) {
				...
				switch (configKey.format || "string") {
					...
					case "json":
						try {
							...
							console.warn(
								`Error parsing JSON in Config key '${configKey.key}' for experiment '${experimentName}': ${e}`
							);
						}
						break;
					default:
						console.warn(
							`Unsupported format '${configKey.format}' in Config key '${configKey.key}' for experiment '${experimentName}'.`
						);
						value = configKey.value;
						break;
				}
				...
				if (key in target) {
					if (isObject(target[key]) && !isObject(value)) {
						console.warn(
							`Config key '${configKey.key}' for experiment '${experimentName}' is overriding object with non-object value.`
						);
					} else if (!isObject(target[key]) && isObject(value)) {
						console.warn(
							`Config key '${configKey.key}' for experiment '${experimentName}' is overriding non-object value with object.`
						);
					}
				}
