export default class Context {
	...
	attribute(attrName, value) {
		this.currentAttributes.push({ name: attrName, value: value.toString(), setAt: Date.now() });
	}

	attributes(attrs) {
		const now = Date.now();
		for (const [attrName, value] of Object.entries(attrs)) {
			this.currentAttributes.push({ name: attrName, value: value.toString(), setAt: now });
		}
	}
	...
	_treatment(experimentName) {
		...
		if (!exposed) {
			this.pendingExposures.push({ name: experimentName, variant, exposedAt: Date.now(), assigned });
			this.pendingCount++;
	...
	_track(goalName, values = null) {
		...
		this.pendingGoals.push({ name: goalName, values, achievedAt: Date.now() });
		this.pendingCount++;
	}
